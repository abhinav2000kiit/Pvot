...
class AnalystProfile extends React.Component {
    ...
    this.state = {
      name:
        ...
        this.props.signinData.user.UserAttributes.filter(item => item.Name === 'name').length > 0
          ...
          : null,
      isEdit: false,
      analyst_followers: this.props.analystDetails ? this.props.analystDetails.followers_count : 0,
      analyst_subscribers: this.props.analystDetails ? this.props.analystDetails.subscriber_count : 0,
  ...
  componentDidMount() {
    ...
    if (this.props.signinData && this.props.userGroup === 'ANALYST' && !this.props.subscriptionPlan) {
      this.props.getSubscriptionPlanList();
    }
    if (this.props.signinData && this.props.userGroup === 'ANALYST' && !this.props.subscriptionPlan) {
      this.props.getAnalystPortfolio();
    }
  ...
  handleLogoutSubmit = event => {
    ...
    Mixpanel.track('Analyst logout clicked - Expert');
  ...
  componentDidUpdate(preState, preProps) {
    ...
    if (this.state.fetchingReferralCode && this.props.referralCode) {
      this.setState({
        fetchingReferralCode: false
      });
      if (navigator.share) {
        navigator.share({
          url: `https://m.pvot.in/referral-code?referralCode=${this.props.referralCode}`,
          text:
            "Hi! I'm inviting you to join Pvot. Pvot is the best stock market app for people looking for expertise on trading and investing."
        });
      }
    }
    if (this.props.message === 'Referral Code Successfull' || this.props.message === 'Invalid Referral Code') {
  ...
  handleName = () => {
    Mixpanel.track('Edit name clicked - Expert');
    this.setState({
      isEdit: true
    });
  };
  ...
  selectFile = event => {
    ...
    Mixpanel.track('Select file button clicked - Expert');
  ...
  checkReferralCode(e) {
    e.preventDefault();
    if (this.state.referralCode.trim().length > 0) {
      this.props.checkReferralCode(JSON.stringify({ referral_code: this.state.referralCode }));
      this.setState({
        ...
        <div style={{ height: '100vh' }}>
          ...
          {!this.state.loading && (
            <Row className='container-fluid mainArea mb-5 py-2'>
              <Col className='center-col-special'>
                  ...
                  <Col className='center-col'>
                    ...
                    <Row className='align-content-center justify-content-center'>
                      ...
                      {this.props.signinData &&
                        ...
                        ((this.props.signinData.user.UserAttributes.filter(item => item.Name === 'name').length > 0 &&
                          !this.props.signinData.user.UserAttributes.filter(item => item.Name === 'name')[0].Value) ||
                          this.props.signinData.user.UserAttributes.filter(item => item.Name === 'name').length ===
                            0) && (
                          <p
                            style={{
                              fontSize: '12px',
                    ...
                    <Row className='align-content-center justify-content-center'>
                      <div
                        onClick={() => {
                          Mixpanel.track('Followers button clicked - Expert');
                          this.props.history.push('/followers', { list: 'Followers' })}}
                        className='col-5'
                      >
                        <Row className='mt-3 justify-content-center'>
                          <div className='followerCount'>
                            {this.props.analystDetails ? this.props.analystDetails.followers_count : 0}
                          </div>
                        </Row>
                        <Row>
                      ...
                      <div className='col-2 mt-3'>
                        <div className='followerSubscriberPartition'></div>
                      </div>
                      <div
                        onClick={() => {
                          Mixpanel.track('Subscribers button clicked - Expert');
                          this.props.history.push('/subscribers', { list: 'Subscribers' })}}
                        className='col-5'
                      >
                        <Row className='mt-3 justify-content-center'>
                          <div className='followerCount'>
                            {this.props.analystDetails ? this.props.analystDetails.subscriber_count : 0}
                          </div>
                        </Row>
                        <Row>
                ...
                <div className='mx-auto p-4'>
                  ...
                  <div className='row my-2 fullProfileButton'>
                    {signinDataName && tradingExp && education ? (
                      <Button
                        ...
                        onClick={() => {
                          Mixpanel.track('Basic details button clicked - Expert');
                  ...
                  <div className='row my-2 fullProfileButton'>
                    {allocationComplete ? (
                      <Button
                        ...
                        className='btn-block rounded-lg py-2 px-1 profileButton'
                        onClick={() => {
                          Mixpanel.track('Allocate trade button clicked - Expert');
                          this.props.history.push({
                  ...
                  <div className='row my-2 fullProfileButton'>
                    {subscriptionComplete ? (
                      <Button
                        ...
                        className='btn-block rounded-lg py-2 px-1 profileButton'
                        onClick={() => {
                          Mixpanel.track('Subscription plan details button clicked - Expert');
                          this.props.history.push({
                      ...
                      <Button
                        ...
                        className='btn-block rounded-lg py-2 px-1 profileButton'
                        onClick={() => {
                          Mixpanel.track('Create Subcription button clicked - Expert');
                          this.props.history.push({
                            pathname: '/create-subscription-plan'
                          });
                        }}
                  ...
                  <Button
                        ...
                        className='btn-block rounded-lg py-2 px-1 profileButton'
                        onClick={() => {
                          Mixpanel.track('Refer Friend button clicked - Expert');
                          this.props.history.push('/refer-friend');
                        }}
                      >
                        <div className='row buttonTextSection'>
                          <div className='col mx-auto buttonText'>Become an Affiliate</div>
                        </div>
                      </Button>
                  </div>
                  {/* <div className='row my-2 fullProfileButton'>
                    <Button
                      variant='outlined'
                      className='btn-block rounded-lg py-2 px-1 profileButton'
                      onClick={() => this.setState({ enterReferralCode: true })}
                    >
                      <div className='row buttonTextSection'>
                        <div className='col mx-auto buttonText'>Enter Referral Code</div>
                      </div>
                    </Button>
                  </div> */}
                </div>
                ...
                <div className=''>
                  <Row className=''>
                    <Col align='center' className=''>
                      <button
                        className='btn btn-link manualButton'
                        onClick={() => {
                          Mixpanel.track('User manual button clicked - Expert');
                ...
                {/* Section 5: FAQ, About and Privacy Policy */}
                <div className='mx-auto mt-5'>
                  <Row className='mx-auto'>
                    <ButtonGroup variant='text' aria-label='text primary button group' align='center'>
                      <span className='bottomButton' onClick={() => {
                        Mixpanel.track('FAQ button clicked - Expert');
                        this.props.history.push('/expert-faqs')}}>
                        FAQs
                      </span>
                      {/* <div className='bottomVerticalLine'></div> */}
                      <span className='bottomButton' onClick={() => {
                        Mixpanel.track('Privacy policy button clicked - Expert');
                        this.props.history.push('/privacy-policy')}}>
                        Privacy Policy
                      </span>
                      {/* <div className='bottomVerticalLine'></div> */}
                      <span className='bottomButton' onClick={() => {
                        Mixpanel.track('Terms of use button clicked - Expert');
              ...
              <Dialog
                open={this.state.enterReferralCode}
                onClose={() => {
                  this.setState({
                    enterReferralCode: false,
                    referralCode: ''
              ...
              >
                <DialogTitle>Enter Your Referral Code</DialogTitle>
                <DialogContent>
                  <Row className='py-2'>
                    <Col>
                      <form onSubmit={this.checkReferralCode.bind(this)}>
                        <Row>
                          <Col>
                            <TextField
                              fullWidth
                              ...
                              readOnly={this.props.message && this.props.message === 'Referral Code Successfull'}
                              error={this.props.message === 'Invalid Referral Code'}
                              helperText={
                                this.props.message === 'Invalid Referral Code'
                                  ? 'Invalid Referral Code'
                                  : this.props.message === 'Referral Code Successfull'
                                  ? 'Verified'
                                  : ''
                              }
                            />
                        ...
                        <Row className='mt-3'>
                          <Col>
                            {!this.state.checkingReferCode ? (
                              <Button
                                type='submit'
                                color='primary'
                                variant='outlined'
                                disabled={this.props.message === 'Referral Code Successfull'}
                                onClick={
                                  this.props.message && this.props.message !== 'Referral Code Successfull'
